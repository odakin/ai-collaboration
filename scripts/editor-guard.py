#!/usr/bin/env python3
"""著者が editor で同じ原稿を開いている日の前検査と carrier commit: 未 commit 変更を exit code で止め、読んだ差分の hash と一致したときだけ著者の変更を commit する。

edit-intent-record.md §7 の規則 7 (carrier は確認した差分と hash で照合) と 規則 9 (前検査は編集と別の tool
呼び出しで、exit code で止める) を 1 本にした道具。

Usage:
  editor-guard.py check   <file> [--repo DIR]            # exit 0 = 未 commit 変更なし (編集してよい)
...
使い方の型 (1 手ずつ別の tool 呼び出しにする):
  1. `editor-guard.py check paper.tex` → exit 0 ならそのまま自分の編集へ。
  2. exit 3 なら表示された差分を読み、著者の編集として妥当かを判断する。
  3. `editor-guard.py carrier paper.tex -m "Author's hand edits (...)" --push` → 読んだ差分と同じなら commit。
     読んだ後にさらに編集が着地していれば exit 4 で止まるので、1 からやり直す。
  4. `check` がもう一度 exit 0 を返してから自分の編集を当てる。

破られ方 (2026-09-13 実測): `git status --short <file> && 編集 && commit` は変更があっても exit 0 なので止まらず、
著者の編集が自分の commit message の commit に入った (2 回)。gate は表示ではなく exit code で判定する。
"""
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        repo = Path(tmp)
        env_git = ["-c", "user.name=t", "-c", "user.email=t@example.org"]
        subprocess.run(["git", "init", "-q", str(repo)], check=True)
        (repo / "paper.tex").write_text("line one\nline two\n", encoding="utf-8")
        subprocess.run(["git", "-C", str(repo), *env_git, "add", "paper.tex"], check=True)
        subprocess.run(["git", "-C", str(repo), *env_git, "commit", "-q", "-m", "init"], check=True)
        os.environ.setdefault("GIT_AUTHOR_NAME", "t"); os.environ.setdefault("GIT_AUTHOR_EMAIL", "t@example.org")
        os.environ.setdefault("GIT_COMMITTER_NAME", "t"); os.environ.setdefault("GIT_COMMITTER_EMAIL", "t@example.org")
...
def main() -> int:
    ...
    c = sub.add_parser("check"); c.add_argument("file"); c.add_argument("--repo", default=".")
