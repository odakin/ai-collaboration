...
- `conventions/delegated-work-packages.md` — how to hand months of analysis to a *collaborator's* AI: judgement (definitions, methods, thresholds) stays with the requester in a spec, execution goes out as one-session work packages whose acceptance criteria are numbers the requester already produced with an independent implementation
...
- [`scripts/README.md`](scripts/README.md) — generated inventory of every script and its docstring-owned purpose. CI rejects a stale inventory, and every script has `--selftest`.
