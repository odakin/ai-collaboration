...
9. <a id="editor-open-precheck"></a>**著者が editor で同じ file を開いている日は、未 commit 変更の検査を編集と別の tool 呼び出しにする。** 「変更が無いことを確かめてから当てる」 を同じ 1 コマンドに書くと、**確かめた瞬間と当てる瞬間の間に着地した著者の編集が、自分の commit に、自分の commit message で入る**。 検査を単独で回して結果を読み、変更があれば**自分の編集を見送って報告する** (carrier commit してから当て直す = 規則 7)。 pre-check の中身は `git diff --quiet -- <file>` で十分。 これは並行 session の staging race (claude-config `multi-session-coordination.md#staging-window-race`) の人間 editor 版で、`git commit -- <path>` では防げない (同じ path なので)。 **検査は表示でなく exit code で止める** — 2026-09-13 に 2 回、`git status --short <file> && 適用 && commit` と書いて著者の編集が自分の commit に入った (`git status` は変更があっても exit 0 = 画面に `M` が出ても鎖は止まらない)。機械化 = [`scripts/editor-guard.py`](../scripts/editor-guard.py): `check <file>` が変更ありで exit 3 (差分を表示し hash を保存)、`carrier <file> -m MSG` は表示した差分と同じ hash のときだけ commit する (規則 7 を兼ねる、読んだ後に着地した編集は exit 4 で止める)。⚠️ 前検査で変更を見つけたら自動で carrier しない — 差分を読む手が抜けると規則 7 が空になる。
