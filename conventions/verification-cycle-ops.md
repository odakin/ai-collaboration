...
2. **再利用できる関数は層1 library へ** (例: `gpt_measurements.py`)、 campaign 側は shim か alias。 判断基準 = 2 campaign 目で同じ形が要ったら (層1 `#second-example-refine`)。 <a id="hoist-synthetic-parameters"></a>**上げる script からは instance の parameter を外す** (2026-09-14): 既定値・usage の例・selftest の pin・docstring に書いた出力値が原稿の parameter とその帰結なら、 公刊版に在る関数形は残して parameter だけ合成値に替え、 pin は差し替えた写しで関数を走らせて出し直す。 定性的な assert (順序・単調性・marginality) が合成値でも成り立つことを selftest で確かめる。 公刊状況は話題でなく**量ごと**に判定する (関数形は公刊版に在り、 値は未公開版にしか無い、 がありうる)。 固有の語・値の候補出しは `claude-config/scripts/scan-private-vocabulary.py --staged` (読む候補であって gate ではない)。 ⚠️ fixture の「合成値」 を書くとき、 直前に外した値を使い回さない (2026-09-14 に 1 回、 commit 前の走査で止めた)。
3. **kernel (定義から独立に導いた一般則・壊れ方) は層1 規約へ** (§ を切るか既存 § に追記)、 instance は private に残す。 汚染を避けるため、 進行中の別 campaign の verdict 方向を漏らす記述は受領後まで待つ (physics-verification-cycle C′)。 **公開層に書く例示にも instance を持ち込まない** (2026-09-11): 名前の leak gate は識別子しか見ないので、 模型固有の数値・構造の数え・原稿の文言は素通りする。 commit 前に instance の term list (固有の数値、 特有の語、 原稿の句) で追加行を走査し、 例示は公刊済みの結果か一般形に置き換える (実例: 盲検 review から上げた kernel の例示に原稿固有の計数と原稿の文言が入り、 公開後に一般化した。 履歴には残る)。 ⚠️ **commit message も同じ公開面**で、 しかも本文と別に直す必要がある (2026-09-12、 同 turn に 2 件): 本文から instance を外しても、 その語を「何を外したか」 として message が再掲すると公開時に相殺される。 走査は `git log @{u}..HEAD --format='%s%n%b'` を term list に通す (= 本文の grep とは別 pass)。 **未 push なら message は書き換えで予防できる** — 「履歴を書き換えない」 は公開済み履歴を壊さないための規則なので未 push commit はその射程外。 判定は `git branch -r --contains <sha>` が空かで見る (`git merge-base --is-ancestor <sha> @{u}` は sha が存在しないときも非 0 を返すので「未 push」 と「無効な ref」 を区別できない)。 書き換えは非対話で閉じる (対象を detached で `commit --amend -F <新 message>` → `git rebase --onto <新 SHA> <旧 SHA> <branch>`)、 合格条件は **tree hash の一致** (`git rev-parse HEAD^{tree}` が前後で同値 = 内容不変が hash で保証される。 `git diff` が空、 より強い)。 上に別 session の commit が積まれているときは freeze を宣言してから一息で回す。 **push の前に reword を入れる** — 順序が逆だと予防の窓が閉じる。 ⚠️ **書き換えの安全網に切った backup branch は、古い message を保持し続ける** (2026-09-12 に実際に残った): 素の `git push` は current branch しか押さない (`push.default` = `simple`) ので直ちには公開されないが、 `--all` / `--mirror` / 全 branch を押す sweep が走れば公開される。 ∴ **backup の役目は tree hash 一致を確認した時点で終わり**、 そこで消す (push の後まで待たない — 待つ間だけ窓が開く。 内容同一が hash で保証されている以上、 backup に固有の復旧価値は「消したい message」 しかない)。 消しても reflog から当面は辿れる。 検証は `git log <branch> --not @{u} --format='%s%n%b'` を term list に通す — **branch の tip だけを見ると、 問題の commit が履歴の途中にあるとき 0 件に見える**。 原則は claude-config [`scientific-computing.md#research-script-homes`](../../claude-config/conventions/scientific-computing.md#research-script-homes)、 名前・PII 側の同型は claude-config `CLAUDE.md` の 2026-06-16 拡張 (規約本文の例と docstring も public surface)。 **逐語の写しは 2026-09-13 から公開 repo の pre-commit と commit-msg の Tier D が止める** (claude-config `scripts/check-unpublished-quote.py`、 [`confidential-repo-boundary.md#unpublished-text-public-gate`](../../claude-config/conventions/confidential-repo-boundary.md#unpublished-text-public-gate))。 言い換えた結果や短い断片は止まらないので、 term list の走査はそのまま要る。 **結果は kernel ではない** (2026-09-14): 定義から導いた未公表の定理・反例・構成・数値は、 一般形に見えても owner が公開と判断するまで上げない (上げるのは方法)。 他人の論文の誤り finding は default 非公開 ([`physics-verification-cycle.md`](physics-verification-cycle.md) §6)。 例示の来歴 (いつ・何件・どの案件) も上げない = claude-config [`CLAUDE.md#owner-activity-facts`](../../claude-config/CLAUDE.md#owner-activity-facts)。 **封じた sandbox / referee session からの hoist は候補を `scratch/hoist-candidates.md` に一般形で書くまで** (2026-09-14): 隔離 worker は公開 repo の file を直接編集せず、 受領側が 5 class を当ててから上げる ([`cold-eyes-isolation.md#post-check`](cold-eyes-isolation.md#post-check) の出口の規律)。
...
6. <a id="hoist-after-ruling"></a>**reviewer の finding は、 受領側の裁定が出るまで規則として上げない** (2026-09-13) — finding は「この原稿のこの箇所がこう見えた」 という観察で、 「一般にこうすべき」 という規則は**受領側 (著者) が採った時に**初めて成立する。 実例: 盲検 review が「長い式の番号が下の行に回る」 を should-fix で報告し、 reviewer session がその日のうちに層1 の LaTeX 規約へ「直すべき症状」 として書いた。 著者の裁定は「完全にカノニカル」 で据え置きだったので、 翌日に規約を反対向きへ訂正する羽目になった (履歴には誤った版が残る)。 ∴ hoist の前に**受領側の裁定記録** (決定 ledger・未手当表) を読み、 裁定済みの finding だけを規則にする。 裁定前に上げてよいのは、 finding の中身に依らない**道具と手順** (再現・検証の方法) だけ。 裁定が出る前に書くしかないなら、 本文に「未裁定の観察」 と明記し、 裁定が出た turn で確定か撤回をする。
