...
| [`expanding-mode-growth.py`](../scripts/expanding-mode-growth.py) (同上) | 膨張背景 (slow roll → 振動) 上の線形 mode 成長を、 **真空を置く時刻** (`--start end|eps_V=1|x=<v>`) と sub-horizon 条件 (`--kmin`) を引数にして数振動ごとに $\max_k\ln n_k$ で出す。 selftest = 厳密 $\epsilon_H=1$ の終点、 結合 0 で成長なし、 結合単調 | [`scientific-computing.md#oscillation-onset-definition`](../../claude-config/conventions/scientific-computing.md#oscillation-onset-definition) |
...
6. **spectator 判定は pivot の質量でなく積算成長で**: conformal 結合 $e^{-\gamma\chi/M_\text{P}}|DH|^2$ の正準場の質量項 $\tfrac{\gamma}{2M_\text{P}}\Box\chi-\tfrac{\gamma^2}{4M_\text{P}^2}\dot\chi^2$ は pivot で $-0.1\gamma H^2$ (小) でも、 slow roll 中の零モードの積算成長は $\int|m^2|/3H^2\,dN=\tfrac{\gamma}{2}\Delta\chi/M_\text{P}$ = **Weyl 因子そのもの** (非正準場が一定なら正準場は $e^{\gamma\Delta\chi/2M_\text{P}}$ 倍)。 $\Delta\chi\sim9M_\text{P}$ なら $\gamma=0.7$ で 24 倍、 末期の質量は $-(2\gamma+\gamma^2/2)H_\text{end}^2$。 「軽い spectator」 は $\gamma\lesssim0.3$ に限る。 質量次元を持つ項 ($\mu^2e^{-\gamma\chi/M_\text{P}}$) は反対符号で指数的に増幅される ([`paper-audit.md#odd-coupling-sign-before-pricing`](../../claude-config/conventions/paper-audit.md#odd-coupling-sign-before-pricing) (e) と対)。
