...
9. **撤回を採用したら、 その撤回が覆わない parameter 領域を同じ anchor script で 1 回走査する (advocate pass)** — 誤り探しの査読を反復すると最小の防御可能な主張に収束し、 計算で退けた regime と定義で退けた regime が同じ顔をする (2026-09 実例: item 8 で「閾値の上は完了しない」 → 「模型外」 と書いた regime が、 scan 範囲を 1 行伸ばすと完了していた = 5 日後に著者の一言で発覚)。 正本 = [`paper-audit.md#advocate-pass-after-retraction`](../../claude-config/conventions/paper-audit.md#advocate-pass-after-retraction)、 値札の符号奇偶 = [`#odd-coupling-sign-before-pricing`](../../claude-config/conventions/paper-audit.md#odd-coupling-sign-before-pricing)。
...
**campaign 3 (sandbox 2 段階の第二の目、 rank-one 族の連続 outcome 一般化、 2026-09-06) からの追補**:

18. **verdict に「証明が実際に使った仮定」 の棚卸しを添える** — statement の仮定 (norm 連続 / 可分距離空間 / 局所有限) と、 証明が本当に使った仮定 (可測性 / 第二可算 / σ-有限 / H の可分性) を分けて書く。 第二の目が別経路で閉じたとき、 両証明の仮定集合の**差**がそのまま「定理をどこまで弱められるか」 と「共有する暗黙前提」 の一覧になる (kernel 15 の続き)。 実測: RN 二表示の経路 ([第 3 経路](#continuous-rank-one-povm-rn-route)) は連続性を可測性まで落とすが H の可分性 (忠実状態・可算稠密集合) を使い、 cell 分割の経路は連続性を本質的に使うが可分性を使わない — 「どちらが強い」 ではなく直交する。 ledger の note には「使った / 使わなかった」 を両方書く (spec が候補に挙げた追加仮定 = 一様非平行性・compact 上の一様連続・有界 outcome 制限・trace class は、 どちらの経路でも不要と判明)。
19. **「同じ証明で一般化できる」 は局所 compact 性の暗黙使用を疑う** — ℝⁿ / ℂ で書かれた証明を一般の可分距離空間へ字面通り運ぶとき、 (a) 「有界 ⇒ μ-有限」 (b) 「有界集合上の一様連続性・sup の達成」 の 2 つは proper (有界閉 = compact) な空間の性質で、 一般には偽 (反例: 離散 ℕ + 数え上げ測度 + ONB は A(X) = 1 だが Tr A(X) = ∞ / 局所 compact でない X では正距離の 2 集合の間で sup|⟨φ_z|φ_w⟩| = 1 になり得る)。 修復は書き下せる (μ-有限開集合の可算基底 → overlap の連続性で sup < 1 となる基底矩形で対角外を可算被覆 → cell を振動 ≤ δ で可算分割 → trace の正規性で可算和)。 「同じ証明で」 は verified と書かず、 転写の gap を item 化して修復 or 仮定 (proper) を明記する (実測: 定理は真、 転写は 2 箇所 gap、 `readings` で ℝⁿ / 一般 X を分けた)。
20. **singleton 主張の solver 無し anchor = Dykstra 射影の一致検査 (heuristic と明記)** — 「JM(A,A) が 1 点」 型の主張は、 周辺条件 (affine 空間、 射影は閉形式) と PSD cone への交互射影 (Boyle–Dykstra) で、 ランダム start から canonical に落ちるかで叩ける。 落ちなければ非 canonical joint の存在 (foil: 平行 pair を挿入すると即座に落ちなくなる、 明示 joint = pair 内の label ランダム化)。 落ちても証明ではない (有限回・certificate なし) — ledger は 👁 のまま、 results に「heuristic」 と書く。 証明級の有限 witness は rank-one 有限 outcome なら pair ごとの値域交叉 (dim = rk a + rk b − rk[a|b] = 0)。 厳密 anchor は SDP (JM(A,A) 上の線形汎函数の最大化) で、 solver の使える環境で置換する。 道具 = `gpt_measurements.py` の `dykstra_self_joint` / `split_rank_one_effect` / `parallel_pair_joint` / `rank_two_density_noncanonical_joint` (rank ≥ 2 密度の反例 model)。
21. **note の定数・係数も item にして数値で叩く** — 証明 note の副次主張 (「c = κ·A(U) が共通下界、 κ = …」) の係数は結論に効かないので誰も検算しない。 Fock 基底 10 行の数値で κ の逆数取り違えが出た (κ_note = min(1, e^{r²}−1) は e^{r²} > 2 で c ≤ A(Uᶜ) を破る、 正は min(1, 1/(e^{r²}−1)))。 結論不変でも、 その note が他 item の根拠なら「修正推奨」 の finding として ledger に残す (kernel 16 の heuristic 文、 kernel 17 の省略と同じ「散文の穴」 の第 3 型 = 定数)。
...
<a id="continuous-rank-one-povm-cell-route"></a>**第 2 経路 (盲検の第二の目、 同日): 極値性も RN も使わない cell 分割** — (i) self-joint B が canonical ⇔ B が対角集合 D の外で消える (B(E) = B(E∩D)、 A′(U) := B(Δ(U)) = B(U×ℂ) = A(U))。 (ii) 正の距離 d にある有界 U₁, U₂ を辺 s ≤ d/(2√2) の正方 cell V_k ∋ z_k, W_l ∋ w_l に分割し、 各 block X = B(E ∩ (V_k×W_l)) ≥ 0 に **有限次元定理** Tr X (1−|⟨z_k|w_l⟩|) ≤ Tr(Q_k^⊥ X) + Tr(Q_l^⊥ X) ≤ Tr(Q_k^⊥ A(V_k)) + Tr(Q_l^⊥ A(W_l)) を当てる (Q^⊥ = 1 − |z⟩⟨z|、 ‖p_z + p_w‖ = 1 + |⟨z|w⟩|)。 rank-one 密度ゆえ Tr(Q_k^⊥ A(V_k)) = (1/π)∫_{V_k}(1 − e^{−|z−z_k|²}) ≤ |V_k| s²/(2π) で、 総和は (|U₁| + |U₂|) s²/(2π) → 0、 一方 1 − |⟨z_k|w_l⟩| ≥ 1 − e^{−d²/8} > 0 は s に依らない ⇒ Tr B(E) = 0。 (iii) D^c は正の距離にある dyadic 正方形の対 Q×Q′ の可算和なので B(D^c) = 0。 使う性質は |⟨z|w⟩| < 1 (z ≠ w) と norm 連続性だけなので、 **norm 連続・pairwise 非平行な rank-one 族 × 局所有限測度の POVM 一般** (atom 可) に同じ証明が通る (= Thm 2 の rank-one 場合の連続 outcome 版; **2026-09-06 に sandbox 盲検の第 3 経路 (下) で verified**、 ただし「同じ証明で」 の字面通りの転写は proper でない X で gap 2 つ = §14 kernel 19)。 この経路の核不等式は上の有限 anchor (`nearly_rank_one_clb_bound`) そのもの (§14 kernel 14)。 経路 1 (極値性 + moment 一意性) との共有前提 = Borel(X²) = Borel(X) ⊗ Borel(X) と effect の有界性のみ。

<a id="continuous-rank-one-povm-rn-route"></a>**第 3 経路 (sandbox 盲検の第二の目、 翌日): 忠実状態の trace 測度に対する RN 密度の二表示 — compact 性・一様性・trace class を使わず、 連続性は可測性で足りる** — 設定: 可分距離空間 X、 局所有限 μ (atom 可)、 可分 H、 単位 vector 族 (φ_z) が z ≠ w ⇒ |⟨φ_z|φ_w⟩| < 1、 A(U) = ∫_U |φ_z⟩⟨φ_z| μ(dz)、 A(X) = 1。 (i) canonical ⇔ B(Δᶜ) = 0 (経路 2 と同じ還元)。 (ii) **支配補題**: 作用素値測度 0 ≤ C ≤ A なら C = ∫ g |φ_z⟩⟨φ_z| dμ、 g: X → [0,1] Borel — 可算稠密な (Q+iQ)-部分空間 S 上の scalar RN 密度 G_{ψχ}(z) が μ-a.e. の z で H 上の有界半正定値形式に延び、 Q(ψ,ψ) ≤ |⟨φ_z|ψ⟩|² から Q(ψ,χ) = g(z)⟨ψ|φ_z⟩⟨φ_z|χ⟩ (Cauchy–Schwarz、 ψ ⊥ φ_z で Q(ψ,·) = 0)。 非平行性はここでは不要。 (iii) 忠実な密度作用素 ρ で τ := tr(ρB(·)) (有限測度、 B ≪ τ)、 V を固定して U ↦ B(U×V) ≤ A(U) に (ii) を当てると dν_{ψχ}/dτ (z,w) = ⟨ψ|φ_z⟩⟨φ_z|χ⟩/r(z) τ-a.e. (r = ⟨φ_z|ρ|φ_z⟩ > 0; 矩形は π-系、 μ は σ-有限)。 第 2 周辺で同じことをすると = ⟨ψ|φ_w⟩⟨φ_w|χ⟩/r(w)。 密度の一意性 + S 稠密 ⇒ |φ_z⟩⟨φ_z|/r(z) = |φ_w⟩⟨φ_w|/r(w) τ-a.e. ⇒ 非平行性で z = w ⇒ τ(Δᶜ) = 0 ⇒ B(Δᶜ) = 0。 外部定理 = scalar RN + 密度の一意性・π-λ・Borel(X²) = Borel(X)⊗Borel(X) (第二可算)・半正定値形式の C–S のみ (disintegration・作用素値 RN・Naimark 拡張・spectral 定理は不要)。 **仮定の棚卸し (kernel 18)**: 連続性は「z ↦ ⟨φ_z|ψ⟩ 可測」 で足りる / 非平行性は pairwise でよく一様性不要 (破れる pair 集合の第 1 射影が μ-零なら結論不変、 正測度で破れると copy 構成 φ_{z+2} = φ_z の swap joint ½(B_c + (id×σ)_*B_c) が反例) / atom 可 / H の可分性を使う (忠実状態・可算稠密集合) / 局所有限性は σ-有限性としてのみ。 副産物: rank-one 密度 |φ_z⟩⟨φ_z|μ と |φ′_w⟩⟨φ′_w|μ′ を持つ 2 つの POVM の任意の joint は {(z,w): φ_z ∥ φ′_w} に集中する。 **rank ≥ 2 で落ちる**: A(U) = |U|·1 (C²) に B(U×V) = |U∩V||0⟩⟨0| + |U||V||1⟩⟨1| が非 canonical joint。 3 経路の共有前提 = Borel(X²) = 積 σ-algebra・effect の有界性・可算加法性。 有限 anchor (μ 純 atomic = 有限 rank-one POVM) = pair ごとの値域交叉 (証明級) + `dykstra_self_joint` (heuristic、 kernel 20)、 ∂̄-witness の閉形式 twin = `husimi_disc_dbar_witness_exact` (Stokes 境界積分 = Euler beta、 kernel 13 の数値 witness を 1 次元閉形式で照合)。
